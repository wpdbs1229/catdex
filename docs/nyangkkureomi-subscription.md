...
무료 사용자는 혼자 쓰는 기본 수집과 기본 꾸미기를 사용할 수 있다.

- 기본 도감 표지 선택
- 기본 골목 배지 확인
- 대표 고양이 1마리 설정
- 기본 프로필 문구 설정
- 내 도감 기준 홈, 도감, 촬영, MY 사용
- 지도 탭에서는 내가 발견한 고양이만 지역 단위로 확인
- 지도 안의 `공유지도 보기`와 사람들의 공개 도감은 `냥꾸러미` 안내 화면으로 연결
...
- 공유 도감 지도에서 동네 단위 공유 고양이 확인
- 동네 도감 랭킹과 사람들의 공개 도감 방문
- 공개 도감 좋아요와 팔로우
...
3. 냥꾸러미 안내
   - 무료 사용자가 지도, 동네 도감 랭킹, 사람들의 공개 도감, 잠금 아이템을 눌렀을 때 표시
   - 구독하면 열리는 기능: 공유 도감 지도, 사람들의 공개 도감, 좋아요/팔로우, 프리미엄 꾸미기
   - 마지막 영역에 구독하기 버튼과 구매 복원 버튼 배치
   - RevenueCat 패키지가 있으면 실제 패키지 가격을 표시
   - RevenueCat 패키지가 없으면 월 3,900원 / 연 39,000원 기준 문구와 준비 상태 안내 표시
...
- 무료 사용자는 앱을 개인 냥도감으로 사용한다.
- 무료 사용자의 촬영, 새 고양이 등록, 내 도감 조회, MY 기본 통계는 막지 않는다.
- 지도 탭은 무료 사용자에게도 열어 두되, 지도 데이터는 내가 발견한 고양이만 보여준다.
- 무료 사용자가 지도 안의 `공유지도 보기`나 사람들의 도감으로 이동하려 하면 `냥꾸러미 안내` 화면을 먼저 보여준다.
- `냥꾸러미` 사용자는 기존 공유 지도, 동네 도감 랭킹, 공개 도감 방문, 좋아요/팔로우를 그대로 사용한다.

클라이언트 단계:

1. `NyangkkureomiUpsellScreen` 추가
   - 공유 기능 잠금 이유와 구독 혜택을 한 화면에서 설명한다.
   - `구독하기` 버튼은 RevenueCat 패키지가 있으면 첫 번째 패키지 구매를 호출한다.
   - 패키지가 없으면 결제 준비 상태 안내를 보여준다.
   - `구매 복원` 버튼을 제공한다.

2. 지도 탭 개인 지도 전환
   - `handleTabChange('map')`은 구독 여부와 관계없이 `MapScreen`을 연다.
   - 기본 지도 데이터는 `cat_encounters`의 내 발견 기록만 집계한 `fetchMyRegions()`를 사용한다.
   - 개인 지도 헤더에 `공유지도 보기` 버튼을 둔다.
   - 무료 사용자가 `공유지도 보기`를 누르면 안내 화면으로 이동한다.
   - 권한이 있으면 기존 공유 지역 데이터인 `fetchRegions()` 기반 공유 지도를 보여준다.

3. 사람들의 도감 게이트
   - `동네 도감 랭킹`, `좋아요한 도감`, `공개 도감 상세` 진입 전에 권한을 확인한다.
   - 무료 사용자는 랭킹 데이터를 불러오지 않고 안내 화면으로 이동한다.
   - 구독 사용자는 기존 조회 API를 호출한다.

4. 잠금 아이템 안내 통합
   - 고양이 서랍에서 프리미엄 표지, 추가 대표 고양이 슬롯, 프리미엄 냥발 도장을 누르면 같은 안내 화면으로 이동한다.
   - Alert 중심 안내를 줄이고 실제 구독 버튼이 있는 화면으로 연결한다.

5. 검증
   - `npm run typecheck`를 통과해야 한다.
   - Android 에뮬레이터에서 무료 상태로 지도 탭 진입 시 내 발견 지도 화면이 보여야 한다.
   - 무료 상태에서 지도 안의 `공유지도 보기`를 누르면 안내 화면이 보여야 한다.
   - 안내 화면에서 `구독하기`, `구매 복원`, `내 도감으로 돌아가기` 버튼이 눌려야 한다.
   - 권한 상태가 `냥꾸러미`일 때 지도와 동네 도감 랭킹이 기존처럼 열려야 한다.

현재 적용 상태:

- `NyangkkureomiUpsellScreen`을 추가해 공유 지도, 사람들의 공개 도감, 좋아요/팔로우, 프리미엄 꾸미기 혜택과 구독 버튼을 한 화면에 표시한다.
- 무료 사용자가 지도 탭으로 진입하면 내 발견 지도만 표시한다.
- 무료 사용자가 지도 안의 `공유지도 보기`, 동네 도감 랭킹, 공개 도감 상세, 좋아요한 도감, 내가 공유한 도감으로 진입하면 공유 기능 대신 안내 화면으로 이동한다.
- `냥꾸러미` 사용자는 기존 공유 지도와 공개 도감 API를 그대로 사용한다.
- 고양이 서랍의 잠금 아이템 안내도 Alert 대신 안내 화면으로 연결한다.

검증 기록:

- `cd catdex-client && npm run typecheck`: 통과.
- Android 개발 빌드 설치: `npx expo run:android` 성공, `com.persimmontree.catdex`가 `Medium_Phone_API_35` 에뮬레이터에 설치되어 실행됨.
- 사용자가 Android 에뮬레이터에서 로그인한 뒤 무료 상태(`무료 서랍`)로 도감 화면 진입 확인.
- 무료 사용자로 하단 `지도` 탭을 누르면 `내 발견 지도` 화면이 표시됨.
- 발견 기록이 없는 계정에서는 `아직 내 발견 지역이 없어요` 빈 상태와 `공유지도 보기` 버튼이 표시됨.
- 무료 사용자로 개인 지도 안의 `공유지도 보기` 버튼을 누르면 `공유 지도는 냥꾸러미에서 열려요` 안내 화면이 표시됨.
- 무료 사용자로 도감 상단 `동네 도감` 버튼을 누르면 실제 랭킹 대신 `사람들의 도감은 냥꾸러미에서 만나요` 안내 화면이 표시됨.
- 안내 화면에는 `공유 도감 지도`, `사람들의 공개 도감`, `좋아요와 팔로우`, `프리미엄 꾸미기`, `구독 준비 상태 보기`, `구매 복원`이 표시됨.
- 현재 로컬 `.env`에 `EXPO_PUBLIC_REVENUECAT_ANDROID_API_KEY`가 없어 결제 상품 로드는 `RevenueCat 공개 SDK 키가 설정되지 않았어요.` 상태로 표시됨.
- 남은 수동 검증은 `냥꾸러미` 권한 사용자로 지도/동네 도감 랭킹이 실제 화면으로 열리는지 확인하는 단계다.
...
`supabase/seed.nyangkkureomi-ab.sql`은 `고양이 서랍`, MY, 도감 표지, 공유 지도, 동네 도감 랭킹 검증용 두 사용자를 만든다. shared-catdex step 6 seed와 같은 방식으로 로컬 Supabase DB 또는 폐기 가능한 QA 프로젝트에서 수동 실행한다. Expo 시뮬레이터에서 실제 Google/Kakao OAuth 계정으로 확인할 때는 seed 상단의 A/B UUID를 `auth.users`의 실제 OAuth 사용자 ID로 바꿔 같은 데이터 모양을 만든다.
...
- 무료 사용자는 `user_entitlements.tier = free`이고 기본 표지 `노을 언덕 도감`을 선택한다.
- `냥꾸러미` 사용자는 `user_entitlements.tier = nyangkkureomi`, `status = active`, `current_period_ends_at = now() + 30 days`이고 프리미엄 표지 `단풍 신사길 도감`을 선택한다.
...
- 반복 적용 시 이전 QA에서 생긴 좋아요/팔로우 상태가 랭킹에 남지 않도록 `collection_likes`, `collection_follows`의 A/B 사용자 관련 행을 초기화한다.

SQL 검증 절차:

1. 모든 마이그레이션을 적용한다.
2. `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f supabase/seed.nyangkkureomi-ab.sql`을 실행한다.
3. `psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -f supabase/verify_nyangkkureomi_ab.sql`을 실행한다.
4. 검증 SQL은 무료 사용자의 `private.user_has_nyangkkureomi()`가 false인지, `냥꾸러미` 사용자가 true인지, 공유 지도용 `cat_regions`가 4건인지, 구독 사용자 랭킹 RPC가 `hasNyangkkureomi = true`와 수집 3마리 상태를 반환하는지 확인한다.
...
- `냥꾸러미` 사용자는 `단풍 신사길 도감` 표지가 선택된 상태로 보이고 프리미엄 표지 잠금이 없어야 한다.
...
- 무료 사용자의 도감 표지는 `무료 서랍 냥도감`, `기본 표지와 대표 고양이 1마리로 꾸민 도감`, `노을 언덕 도감` 기준으로 보여야 한다.
- `냥꾸러미` 사용자의 도감 표지는 `냥꾸러미 서랍 도감`, `프리미엄 표지와 주인공 3마리를 진열한 도감`, `단풍 신사길 도감` 기준으로 보여야 한다.
...
- 공유 지도와 동네 도감 랭킹 진입은 `App.tsx`의 `hasNyangkkureomiAccess`로 막는다. 이 값은 DB의 `user_entitlements`와 RevenueCat 복원 상태를 함께 본다.
- 공유 지도는 권한이 있을 때 `MapScreen mode="shared"`로 진입해 `fetchRegions()` 결과를 보여준다.
- 동네 도감 랭킹은 권한이 있을 때 `NeighborhoodRankingScreen`으로 진입하고 `list_public_collection_rankings()` RPC 결과를 보여준다.

공유 지도/동네 도감 랭킹 수동 검증:

1. `냥꾸러미` 사용자로 앱에 로그인한다. OAuth 계정으로 확인할 때는 seed의 `40000000-0000-0000-0000-0000000000b2`를 실제 `auth.users.id`로 바꿔 적용한다.
2. 지도 탭에서 `공유지도 보기`를 누른다. `냥꾸 A/B 골목 근처`, `냥꾸 A/B 공원 근처`가 보이고 `냥꾸러미` 안내 화면으로 이동하지 않아야 한다.
3. 도감 탭에서 상단 `동네 도감`을 누른다. `동네 도감 랭킹` 화면이 열리고 `냥꾸러미 서랍 도감` 항목이 `냥꾸러미 사용 중`, 수집 3마리, 골목 배지 4개 기준으로 보여야 한다.
4. 랭킹 항목을 눌러 공개 도감 상세로 들어간다. 무료 사용자의 공개 도감도 상세 진입이 가능해야 하고, 좋아요/팔로우 버튼이 동작해야 한다.
5. 무료 사용자로 같은 버튼을 누르면 공유 지도는 `공유 지도는 냥꾸러미에서 열려요`, 동네 도감 랭킹은 `사람들의 도감은 냥꾸러미에서 만나요` 안내 화면으로 이동해야 한다.
