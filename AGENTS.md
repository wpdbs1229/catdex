...
- `catdex-client`: Expo React Native + TypeScript 모바일 앱입니다. 현재 재작성 대기 상태입니다.
...
외주 디자인 개편에 맞춰 앱을 처음부터 다시 만듭니다. 기존 화면 코드는 모두 제거했습니다.

**남아 있는 것**

- `supabase/`: 마이그레이션과 RLS 정책 전체
- `catdex-client/src`: 재활용하기로 한 코드만 남았습니다.
...
- 모든 화면과 컴포넌트, 로그인·지도 외의 API 레이어
- CatVision 네이티브 모듈(`modules/cat-vision`)과 TS 브리지
...
- 촬영·매칭 파이프라인을 다시 만들 때는 `domain-rules.md`의 "온디바이스 비전 계약"을 먼저 확인합니다. 스키마의 `embedding`·`embedding_version` 컬럼은 남아 있고, 이를 채울 모듈만 없는 상태입니다.
- 새 폴더 구조와 화면/수동 QA 기준은 디자인이 확정되면 다시 정합니다.
