...
export function CollectionCoverHeader({ collectionTheme, onCustomize, onExplore, profile, progress, summary }: CollectionCoverHeaderProps) {
  ...
  return (
    <View style={[styles.cover, collectionPaletteStyle(collectionTheme?.palette)]}>
        ...
        <View style={styles.selectedStampRow}>
          {selectedStamps.map((stamp) => (
            <View key={stamp.id} style={styles.selectedStampPill}>
              <Text style={styles.selectedStampIcon}>◦</Text>
      ...
      <View pointerEvents="none" style={styles.decorativeWash} />
      <Star color="rgba(120, 91, 63, 0.2)" fill="rgba(120, 91, 63, 0.12)" size={72} style={styles.decorativeStar} />
    </View>
  );
}
