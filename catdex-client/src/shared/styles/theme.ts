...
/**
 * 외주 디자인 시안 기준 팔레트.
 * 흰 배경 위에 따뜻한 주황을 주 강조색으로 쓰고, 좋아요처럼 시선을 끌어야 하는
 * 지점에만 코랄 레드를 쓴다. 두 색을 같은 위계로 섞지 않는 것이 이 시스템의 규칙이다.
 *
 * 활성 탭은 원래 코랄 레드였지만 주황으로 옮겼다(nd.colors.accent). 하단바가 홈
 * 화면의 사원증·인사고과 바로 아래에 붙어 있어, 거기서만 빨강이면 강조가 두 갈래로
 * 갈라져 보인다.
 */
export const theme = {
  colors: {
