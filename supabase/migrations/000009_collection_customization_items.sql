insert into public.collection_themes (id, name, description, palette, is_premium, sort_order) values
  ('spring-flower-alley', '봄꽃 골목', '꽃잎이 흩날리는 계절별 감성의 무료 표지', 'spring', false, 30),
  ('summer-awning-shade', '여름 처마 그늘', '뜨거운 햇빛을 피해 쉬는 여름 골목 표지', 'summer', false, 40),
  ('storybook-forest-path', '손그림 숲길', '따뜻한 손그림 판타지 감성의 냥꾸러미 표지', 'storybook', true, 140),
  ('pastel-paw-drawer', '파스텔 발자국', '아기자기한 스티커와 발자국을 담은 냥꾸러미 표지', 'pastel', true, 150),
  ('prague-rooftop', '프라하 지붕', '붉은 지붕과 작은 광장이 떠오르는 중유럽 감성 표지', 'europe', true, 160),
  ('moonlight-lamp-alley', '달빛 골목', '가로등 아래 조용히 빛나는 밤 산책 표지', 'moon', true, 170)
...
insert into public.season_stamps (id, name, description, season_key, starts_on, ends_on, is_premium, sort_order) values
  ('rainy-day-2026', '비 오는 날 냥발 도장', '비 오는 골목에서 만난 순간을 남겨요.', '2026-rainy', date '2026-06-01', date '2026-07-15', false, 25),
  ('flower-road-2026', '꽃길 냥발 도장', '꽃잎 사이로 지나간 발자국을 모아요.', '2026-spring', date '2026-03-01', date '2026-05-31', true, 35),
  ('moon-patrol-2026', '달밤 순찰 냥발 도장', '달빛 아래 만난 고양이를 기억해요.', '2026-autumn', date '2026-09-01', date '2026-11-30', true, 45),
  ('old-square-2026', '작은 광장 냥발 도장', '중유럽 감성의 작은 광장 산책 기록이에요.', '2026-winter', date '2026-12-01', date '2027-02-28', true, 50)
on conflict (id) do update set
  name = excluded.name,
  description = excluded.description,
