...
as $$
  with period_encounters as (
    ...
    where encounters.region_name = p_region_name
      ...
      and encounters.seen_at >= current_date - (least(greatest(coalesce(p_days, 30), 1), 365) - 1)
  ),
  ...
  ranked as (
    select
      ...
      coalesce(user_scores.user_id = (select auth.uid()), false) as is_me
    from user_scores
  ...
  select
    ...
    ranked.is_me
  from ranked
  where ranked.rank <= least(greatest(coalesce(p_limit, 5), 1), 50)
